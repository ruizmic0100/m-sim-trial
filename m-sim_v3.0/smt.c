...
// This vector holds the specific extra integer register counts
int int_reg_cnts[8] = {0};
...
DCRA_int_register_cap(32),
dlite_evaluator(NULL), 
sleep(0), interrupts(0), entry_point(0), waiting_for(0), nfds(0), next_check(0),
pending_signal(0), sigmask(0)
{
	memset(sigaction_array,0,34);
	memset(sigsignal_array,0,34);
	memset(sigflag_array,0,34);
};
...
context::context(const context & source)
{
	...
	DCRA_int_register_cap = source.DCRA_int_register_cap;
	...
	pending_signal = source.pending_signal;
	sigmask = source.sigmask;

	memcpy(sigaction_array,source.sigaction_array,34);
	memcpy(sigsignal_array,source.sigsignal_array,34);
	memcpy(sigflag_array,source.sigflag_array,34);
}
