...
// The extra integer register cap
static int int_reg_cap;

extern int int_reg_cnts[];
...
public:
	...
	*/
	...
	//Pending RPCC: If the instruction RPCC is called we need to get the correct cycle count, there are two problems
	//a) In simulation execution occurs at rename time (we want it to occur at execute time)
	//b) The rename of successive instructions will use the result from RPCC at rename.
	//If the RPCC is not in spec_mode, when it is executed (must check for dependencies):
	//a) The equivalent of, flush the remaining ROB, rename all flushed instructioned (this would now have the correct value).
	//We were going to store RPCCs here... we don't seem to need to. Remove this on 12/01/2010 or when RPCC handling is complete.
	...
	//Signal data (there are only 32 signals but we have to move one to signal 33).
	qword_t sigaction_array[34], sigsignal_array[34], sigflag_array[34];
	int pending_signal;
