...
void mysrand(unsigned int seed)			//random number generator seed
{
...
//get a random number
int myrand()					//returns random number
{
...
int mystricmp(const char *s1, const char *s2)	//strings to compare, case insensitive
{
	unsigned char u1, u2;
...
//bit, for example, extractl(word, 6, 3) w/ 8 bit word = 01101011 returns 00000110
...
//this function computes the decimal low-order digit of the number pointed to by valptr,
//and returns this digit after dividing *valptr by ten; this function is called ONLY to
//compute the low-order digit of a long whose high-order bit is set
...
char * myvsprintf(char *obuf, const char *format, va_list v)
{
	...
	int count(0);				//counts output characters
	int fcode(0);				//format code
	int width(0), prec(0);			//field width and precision
	int lzero(0);				//number of padding zeroes required on the left and right
	int prefixlength(0);			//length of prefix
	int otherlength(0);			//combined length of leading zeroes, trailing zeroes, and suffix
...
#define MAXDIGS		22			//maximum number of digits in printable number

	char *bp, *p;				//starting and ending points for value to be printed
	const char *bpnull = "(null)";
	int k, lradix, mradix;			//work variables
	const char *prefix;			//pointer to sign, "0x", "0X", or empty
	//values are developed in this buffer
	static char buf[MAXDIGS*4], buf1[MAXDIGS*4];
	char *tab;				//pointer to a translate table for digits of whatever radix
	slargeint_t val;			//value being converted, if integer
	dfloat_t fval;				//value being converted, if floating point
	...
	for(;;)
	{
		...
		// % has been found, the following switch is used to parse the format specification and to
		//perform the operation specified by the format letter; the program repeatedly goes back
		//to this switch until the format letter is encountered
...
fixed:
		...
		case 's':
			...
			if(bp == NULL)
			{
				//FIXME: technically unsafe, not verified to be safe
				bp = const_cast<char *>(bpnull);
			}
			p = bp + strlen(bp);
			break;
...
sqword_t myatosq(char *nptr, char **endp, int base)
{
	...
	//store in ENDP the address of one character past the last character we converted
	if(endp != NULL)
		*endp = (char *) s;
...
FILE * gzopen(const char *fname, const char *type)
{
	...
	if(ext != NULL && *ext != '\0')
	{
		for(unsigned int i=0; i < N_ELT(gzcmds); i++)
		{
...
word_t crc(word_t crc_accum, word_t data)
{
  ...
  for (unsigned int j=0; j < sizeof(word_t); j++)
    {
