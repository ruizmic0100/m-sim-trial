...
// /**
//  * @brief Set the reg cap on all threads.
//  * 
//  * @param int_reg_cap 
//  * @return int* returns back the reg_cap so that the odb macro doesn't edit anything.
//  */
// int* set_reg_cap(int int_reg_cap)
// {
// 	for (int i=0; i < num_contexts; i++) {
// 		int_reg_cnt.at(i) = int_reg_cap;
// 	}
...
void sim_reg_options(opt_odb_t *odb)
{
	...
	// Apply this to all threads. lambda expression
	opt_reg_int(odb, "-int_reg_cap", "",
		"Caps the amount of int registers.",
		&int_reg_cap, /* default */ 32,
		/* print */ TRUE, /* format */ NULL);
...
//						}
		...
		{
			if(cores[core_num].reg_file.find_free_physreg(my_regs.dest) < 0 || (contexts[disp_context_id].DCRA_int_rf >= contexts[disp_context_id].DCRA_int_register_cap))
			{
			...
			if((out1 != 0) && (out1 != 32))
			{
				...
				else
				{
					if(my_regs.dest==REG_INT)
					{
						...
						int_reg_cnts[disp_context_id]++;
						printf("total extra register usage: %i\n", int_reg_cnts[0] + int_reg_cnts[1] + int_reg_cnts[2] + int_reg_cnts[3]);
						// if (contexts[disp_context_id].DCRA_int_rf > 32) printf("DCRA_int_rf: %i\n", contexts[disp_context_id].DCRA_int_rf);
					}
					else
					{
						/******* DCRA *********/
