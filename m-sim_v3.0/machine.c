...
#if 0
//cross-endian execution now works with EIO trace files only...
...
const md_reg_names_t md_reg_names[] =
{
	/* name */	/* file */	/* reg */
...
//returns a register name string, NULL if not found
	...
	eval_value_t *val)			//input, output
{
	...
	case rt_gpr:
		...
		if(!is_write)
		{
			...
			regs->regs_R[reg] = eval_as<qword_t>(*val);
		break;
	case rt_lpr:
		...
		{
			val->type = et_qword;
			val->value.as_qword = regs->regs_F[reg].q;
		}
		else
			regs->regs_F[reg].q = eval_as<qword_t>(*val);
		break;
	case rt_fpr:
		if(reg < 0 || reg >= MD_NUM_FREGS)
			return "register number out of range";
		...
		{
			val->type = et_double;
			val->value.as_double = regs->regs_F[reg].d;
		}
		else
			regs->regs_F[reg].d = eval_as<double>(*val);
		break;
	case rt_ctrl:
		switch(reg)
		{
		case 0:	//FPCR
			if(!is_write)
			{
				...
				regs->regs_C.fpcr = eval_as<qword_t>(*val);
			break;
		case 1:	//UNIQ
			if(!is_write)
			{
				...
				regs->regs_C.uniq = eval_as<qword_t>(*val);
			break;
		default:
			return "register number out of range";
		}
...
{
	myfprintf(stream, "%4s: %16ld/0x%012lx", md_reg_name(rt_gpr, reg), regs[reg], regs[reg]);
}

void md_print_iregs(std::vector<md_gpr_t> & regs, FILE *stream)
{
	for(int i=0; i < MD_NUM_IREGS; i += 2)
	{
...
{
	myfprintf(stream, "%4s: %16ld/0x%012lx/%f", md_reg_name(rt_fpr, reg), regs[reg].q, regs[reg].q, regs[reg].d);
}

void md_print_fpregs(std::vector<md_fpr_t> & regs, FILE *stream)
{
	for(int i=0; i < MD_NUM_FREGS; i += 2)
	{
...
word_t md_xor_regs(regs_t *regs)
{
	...
	for(int i=0; i < (MD_NUM_IREGS-1); i++)
	{
		checksum ^= regs->regs_R[i];
	}

	for(int i=0; i < (MD_NUM_FREGS-1); i++)
	{
		checksum ^= regs->regs_F[i].q;
	}
