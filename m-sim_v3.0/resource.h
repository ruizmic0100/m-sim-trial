...
{
	public:
		int rclass;			//matching resource class: insts with this resource class will be able to execute on this unit
...
class res_pool
{
	public:
		...
		res_template *res_get(int rclass);

		//dump the resource pool POOL to stream STREAM
		void res_dump(FILE *stream);
};
