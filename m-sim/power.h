...
//The following are things you might want to change when compiling

//The output can be in 'long' format, which shows everything, or 'short'
//format, which is just what a program like 'grap' would want to see
#define LONG 1
#define SHORT 2
...
//FUDGEFACTOR 1.0 -> 0.8um (800nm)
 
//#define FUDGEFACTOR 1.0
#define FUDGEFACTOR 12.307692307692307692307692307692
...
//CMOS 0.8um model parameters - from Appendix II of Cacti tech report
...
//note that the value of Cgatepass will be different depending on whether or not
//the source and drain are at different potentials or  the same potential.
...
//Threshold voltages (as a proportion of Vdd). If you don't know them, set all values to 0.5
...
public:
	...
	double dcache3_decoder;
	double dcache3_wordline;
	double dcache3_bitline;
	double dcache3_senseamp;
	double dcache3_tagarray;

	double total_power;
	//What is nodcache2 for? It isn't clear from its usage in power.c
	...
	double dcache3_power;

	double clock_power;
};

//Used to pass values around the program
#include "cacti/data.h"
typedef cacti_parameter_type time_parameter_type;
typedef cacti_result_type time_result_type;

//The following block now sources from cacti/data.h instead, it is typedefed above.
#if 0
...
void power_reg_stats(stat_sdb_t *sdb);
time_result_type cacti_calculate_time(time_parameter_type*);
void cacti_output_data(time_result_type*, time_parameter_type*, FILE * output);
int pop_count(quad_t bits);
int pop_count_slow(quad_t bits);

class processor_power {
public:
	processor_power();

	unsigned int core_id, contexts_on_core;
	power_result_type power;
	int *bimod_config,*twolev_config,*comb_config,*btb_config;
	int ras_size;
	cache_t *cache_dl1,*cache_il1,*cache_dl2,*cache_dl3,*dtlb,*itlb;

	int opcode_length,inst_length;

	int decode_width,issue_width,commit_width;
	int ROB_size,rf_size,iq_size,LSQ_size;
	int data_width;
	int res_ialu,res_fpalu,res_memport;

	int nvreg_width,npreg_width;

	double global_clockcap;

	//These vectors replace the old scheme where, for NAME
	//double NAME, NAME1, NAME2, NAME3
	//These will be size 4, [0]->NAME, [1]->NAME1, [2]->NAME2, [3]->NAME3
	std::vector<double> rename_power;
	std::vector<double> bpred_power;
	std::vector<double> window_power;
	std::vector<double> lsq_power;
	std::vector<double> regfile_power;
	std::vector<double> icache_power;
	std::vector<double> dcache_power;
	std::vector<double> dcache2_power;
	std::vector<double> dcache3_power;
	std::vector<double> alu_power;
	std::vector<double> falu_power;			//falu_power was not included in clock-gating
	std::vector<double> resultbus_power;
	std::vector<double> clock_power;

	std::vector<double> total_cycle_power;

	//These do not use [0], only had data for clock-gating
	std::vector<double> last_single_total_cycle_power;
	std::vector<double> current_total_cycle_power;
	std::vector<double> max_cycle_power;
	...
	counter_t dcache_access,dcache2_access,dcache3_access;
	counter_t alu_access,ialu_access,falu_access;
	...
	counter_t total_dcache_access,total_dcache2_access,total_dcache3_access;
	...
	counter_t max_dcache_access,max_dcache2_access,max_dcache3_access;
	...
	//Calculate power and output to a file stream if desired
	void calculate_power(FILE * output);
...
private:
	...
	double squarify_new(int rows, int cols);
	void dump_power_stats(FILE * output);
