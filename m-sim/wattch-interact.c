...
//this is all needed to satisfy externs from power.c, but is not used in interactive mode
...
vector<cache_t> cache_dl1;
vector<cache_t> cache_il1;
vector<cache_t> cache_dl2;
vector<cache_t> dtlb;
vector<cache_t> itlb;

int btb_config[2] =	{ 512, 4 };		//{nsets, assoc}
int twolev_config[4] =	{ 1, 1024, 8, FALSE};	//{l1size, l2size, hist, xor}
int bimod_config[1] =	{ 2048 }; 		//{bimod_tbl_size}
int comb_config[1] =	{ 1024 };		//{meta_table_size}

int num_regs;

void cacti_calculate_time(time_result_type *result,time_parameter_type *parameters);
void cacti_output_data(time_result_type *result,time_parameter_type *parameters);

int main()
{
	processor_power power;
	...
	case 1:
		...
		printf("%d-entryx%d-width,%d-rdport,%d-wrport: %f (W)\n",num_regs,data_width,rports,wports,power.simple_array_power(num_regs,data_width,rports,wports,0));
		printf(" decode_power (W): %f\n",power.simple_array_decoder_power(num_regs,data_width,rports,wports,0));
		printf(" wordline_power (W): %f\n",power.simple_array_wordline_power(num_regs,data_width,rports,wports,0));
		printf(" bitline_power (W): %f\n",power.simple_array_bitline_power(num_regs,data_width,rports,wports,0));
		break;
	case 2:
		...
		printf("%d-entryx%d-width,%d-rdport,%d-wrport: %f (W)\n",num_regs,data_width,rports,wports,power.simple_array_power(num_regs,data_width,rports,wports,1));
		printf(" decode_power (W): %f\n",power.simple_array_decoder_power(num_regs,data_width,rports,wports,1));
		printf(" wordline_power (W): %f\n",power.simple_array_wordline_power(num_regs,data_width,rports,wports,1));
		printf(" bitline_power (W): %f\n",power.simple_array_bitline_power(num_regs,data_width,rports,wports,1));
		break;
	case 3:
		...
		printf("%d-entryx%d-tagwidth,%d-rdport,%d-wrport: %f (W)\n",num_regs,data_width,rports,wports,power.cam_array(num_regs,data_width,rports,wports));
		printf(" tagdrive_power (W): %f\n",power.cam_tagdrive(num_regs,data_width,rports,wports));
		printf(" tagmatch_power (W): %f\n",power.cam_tagmatch(num_regs,data_width,rports,wports));
		break;
	case 4:
	{
		int nsets, bsize, assoc,res_memport, tagsize;
		int ndwl, ndbl, nspd, ntwl, ntbl, ntspd, c,b,a,cache, rowsb, colsb, trowsb, tcolsb;
		double cache_decoder, cache_wordline, cache_bitline, cache_senseamp, cache_tagarray, total_cache_power;
		...
		cacti_calculate_time(&time_result,&time_parameters);
		cacti_output_data(&time_result,&time_parameters);
		...
		cache_decoder = res_memport*ndwl*ndbl*power.array_decoder_power(rowsb,colsb,predeclength,1,1,cache);
		cache_wordline = res_memport*ndwl*ndbl*power.array_wordline_power(rowsb,colsb,wordlinelength,1,1,cache);
		cache_bitline = res_memport*ndwl*ndbl*power.array_bitline_power(rowsb,colsb,bitlinelength,1,1,cache);
		cache_senseamp = res_memport*ndwl*ndbl*power.senseamp_power(colsb);
		cache_tagarray = res_memport*ntwl*ntbl*(power.simple_array_power(trowsb,tcolsb,1,1,cache));
