...
void core_t::rollbackTo(context & target, counter_t & sim_num_insn, ROB_entry * targetinst, bool branch_misprediction, int int_reg_cnts[])
{
	...
	while(&target.ROB[ROB_index]!=targetinst)
	{
		...
		if(target.ROB[ROB_index].physreg >= 0){
			...
			if(target.ROB[ROB_index].dest_format == REG_INT)
			{
				target.DCRA_int_rf--;
				assert(target.DCRA_int_rf >= 0);
				int_reg_cnts[target.ROB[ROB_index].context_id]--;
			}
			else
			{
				target.DCRA_fp_rf--;
