...
//get a free resource from resource pool POOL that can execute a operation of class CLASS, returns a pointer to the resource template, returns NULL
//if there are currently no free resources available, follow the MASTER link to the master resource descriptor;
//NOTE: caller is responsible for reseting the busy flag in the beginning of the cycle when the resource can once again accept a new operation
res_template * res_pool::res_get(int rclass)
{
	//must be a valid class and there must be at least one resource in this class
	assert((rclass < MAX_RES_CLASSES) && table[rclass][0]);

	for(int i=0; i<MAX_INSTS_PER_CLASS; i++)
	{
		//If the resource exists (not NULL) and is not busy
		if(table[rclass][i] && !table[rclass][i]->master->busy)
		{
			return table[rclass][i];
		}
		else
		{
			//Once we get to NULL, we have to stop
			return NULL;
		}
	}
	return NULL;
...
void res_pool::res_dump(FILE *stream)
{
	if(!stream)
	{
		stream = stderr;
	...
	for(int i=0; i<MAX_RES_CLASSES; i++)
	{
		...
		for(j=0; j<MAX_INSTS_PER_CLASS; j++)
		{
			...
			fprintf(stream, "\t%s (busy for %d cycles) ", table[i][j]->master->name,table[i][j]->master->busy);
		}
		assert(j == nents[i]);
		fprintf(stream, "\n");
	}
