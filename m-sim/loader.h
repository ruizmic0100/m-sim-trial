...
class loader_t
{
	...
	private:
		//Handle loading file into simulated memory based on file type.
		//Provide the source file as well as register and memory state. As well as environment, arguments and zero_bss_segs
		int load_ecoff_littlealpha(FILE * fobj, regs_t * regs, mem_t * mem, std::vector<std::string> argv, std::vector<std::string> envp, int zero_bss_segs);
		int load_elf(FILE * fobj, regs_t * regs, mem_t * mem, std::vector<std::string> argv, std::vector<std::string> envp, int zero_bss_segs);
		int memory_action(mem_cmd action, mem_t * mem, md_addr_t addr, void *vp, int nbytes);
};
#endif
