...
int loader_t::memory_action(mem_cmd action, mem_t * mem, md_addr_t addr, void *vp, int nbytes)
{
	mem->mem_bcopy(action, addr, vp, nbytes);
	...
	temp.size = nbytes;
	temp.protections = 7;
	temp.flags = OSF_MAP_INHERIT;

	mem->internal_map.push_back(temp);
	return 0;
}

//Support for reading in Elf:  Weidan Wu 10/23/2010, Modified for M-Sim Release November 2010
int loader_t::load_elf(FILE * fobj, regs_t * regs, mem_t * mem, std::vector<std::string> argv, std::vector<std::string> envp, int zero_bss_segs)
{
   	Elf64_External_Ehdr Ehdr;
   	Elf64_External_Phdr Phdr;

	const char * filename = argv[0].c_str();
	unsigned long long text_offset = 0;
	unsigned long long data_offset = 0;

	//perform the ELF load
	if(fread(&Ehdr, sizeof(Elf64_External_Ehdr), 1, fobj) < 1)
		fatal("cannot read ELF header from executable `%s'", filename);

	if((Ehdr.e_type != ET_EXEC) && (Ehdr.e_type != ET_REL))
	{
		fprintf(stderr, "bad file type, '%s' not executable\n", filename);
		fatal("bad file type, '%s' not executable", filename);
	}

	if(Ehdr.e_machine != EM_ALPHA)
		fatal("bad architecture type, '%s' not ALPHA binary", filename);

//	fprintf(stderr,"ident(%s): type(%x) machine(%x) entry(%llx) phoff(%llx) shoff(%llx) flags(%x) ehsize(%x) phentsize(%x) phnum(%x) shentsize(%x) shnum(%x) shstrndx(%x)\n",Ehdr.e_ident, Ehdr.e_type, Ehdr.e_machine, Ehdr.e_entry, Ehdr.e_phoff, Ehdr.e_shoff, Ehdr.e_flags, Ehdr.e_ehsize, Ehdr.e_phentsize, Ehdr.e_phnum, Ehdr.e_shentsize, Ehdr.e_shnum, Ehdr.e_shstrndx);

	mem->ld_prog_entry = Ehdr.e_entry;

	long phdr_pos = ftell(fobj);

	//read the program headers
	for(size_t j=0; j<Ehdr.e_phnum; j++)
	{
		if(fseek(fobj, phdr_pos + (j * sizeof(Elf64_External_Phdr)),SEEK_SET) == -1)
			fatal("could not reset Phdr location");

		if(fread(&Phdr, sizeof(Elf64_External_Phdr), 1, fobj) < 1)
			fatal("cannot read program header from executable '%s'", filename);
//		fprintf(stderr,"%ld: type(%x) flags(%x) offset(%llx) vaddr(%llx) paddr(%llx) filesz(%llx) memsz(%llx) align(%llx)\n",j, Phdr.p_type, Phdr.p_flags, Phdr.p_offset, Phdr.p_vaddr, Phdr.p_paddr, Phdr.p_filesz, Phdr.p_memsz, Phdr.p_align);

		char *p = (char*)calloc(Phdr.p_filesz,sizeof(char));
		if(!p)
		{
		...
		if(fread(p, Phdr.p_filesz, 1, fobj) < 0)
			fatal("could not read %s segment from executable", "??");
//		mem->mem_bcopy(Write, Phdr.p_vaddr, p, Phdr.p_filesz);
		memory_action(Write, mem, Phdr.p_vaddr, p, Phdr.p_filesz);
		free(p);
		...
		if(Phdr.p_flags & 1)
		{
			...
			{
				continue;
				fatal("text already loaded :-(\n");
			}
			mem->ld_text_base = Phdr.p_vaddr;
			mem->ld_text_size = Phdr.p_filesz;
			//mem->ld_text_mem_size = Phdr.p_memsz;
			text_offset = Phdr.p_offset;
		}
		else
		{
//			if(mem->ld_data_base)
//				fatal("data already loaded :-(\n");
			mem->ld_data_base = Phdr.p_vaddr;
			mem->ld_data_size = Phdr.p_filesz;
			//mem->ld_data_mem_size = Phdr.p_memsz;
			data_offset = Phdr.p_offset;
		}
	}

//	fprintf(stderr,"%llx text_base\n",mem->ld_text_base);
//	fprintf(stderr,"%llx text_size\n",mem->ld_text_size);
//	fprintf(stderr,"%llx data_base\n",mem->ld_data_base);
//	fprintf(stderr,"%llx data_size\n",mem->ld_data_size);

//	assert(mem->ld_text_base);
//	assert(mem->ld_text_size);
//	assert(mem->ld_data_base);
//	assert(mem->ld_data_size);

	if((Ehdr.e_type != ET_EXEC) && (Ehdr.e_type != ET_REL))
	{
		mem->ld_text_base = 0xe00;
		mem->ld_text_size = 0x17fb0;
		mem->ld_data_base = 0x30060;
		mem->ld_data_size = 0x20;
	...
	char *p = (char*)calloc(mem->ld_text_size,sizeof(char));
	if(!p)
	{
		fprintf(stderr,"out of virtual memory");
		return ENOMEM;
	}
	if(fseek(fobj,text_offset,SEEK_SET) == -1)
		fatal("could not reset location in executable");
	if(fread(p, mem->ld_text_size, 1, fobj) < 1)
		fatal("could not read %s segment from executable", "text");
	mem->mem_bcopy(Write, mem->ld_text_base, p,mem->ld_text_size);
	//FIXME if mem->ld_text_m`em_size > mem->ld_text_size
	free(p);

	if(mem->ld_data_base == 0)
	{
		mem->ld_data_base = mem->ld_text_base;
		mem->ld_data_size = mem->ld_text_size;
		fprintf(stderr,"Corrected %llx data_base\n",mem->ld_data_base);
		fprintf(stderr,"Corrected %llx data_size\n",mem->ld_data_size);
	}
	else
	{
		p = (char*)calloc(mem->ld_data_size,sizeof(char));
		if(!p)
		{
			fprintf(stderr,"out of virtual memory");
			return ENOMEM;
		}
		if(fseek(fobj,data_offset,SEEK_SET) == -1)
			fatal("could not reset location in executable");
		if(fread(p, mem->ld_data_size, 1, fobj) < 1)
			fatal("could not read %s segment from executable", "data");
		mem->mem_bcopy(Write, mem->ld_data_base, p,mem->ld_data_size);
		//FIXME if mem->ld_data_mem_size > mem->ld_data_size
		free(p);
	}
...
int loader_t::load_ecoff_littlealpha(FILE * fobj, regs_t * regs, mem_t * mem, std::vector<std::string> argv, std::vector<std::string> envp, int zero_bss_segs)
{
	ecoff_filehdr fhdr;
	ecoff_aouthdr ahdr;
	ecoff_scnhdr shdr;
	...
	{
		fprintf(stderr,"cannot read header from executable `%s'", filename);
		return EFAULT;
	}

	if(fread(&ahdr, sizeof(ecoff_aouthdr), 1, fobj) < 1)
	{
		fprintf(stderr,"cannot read AOUT header from executable `%s'", filename);
		return EACCES;
	}
	...
	if(fhdr.f_flags & 0x3000)
	{
		...
		if(fclose(fobj))
		{
			fprintf(stderr,"could not close executable `%s'", filename);
			return EIO;
		}
...
//		argv.insert(argv.begin(),"../sysfiles/alpha-sys-root/sbin/loader");
		argv.insert(argv.begin(),"/home/jloew/.nfs/Desktop/m-sim-c++/sysfiles/alpha-sys-root/sbin/loader");
...
//		return ld_load_prog("../sysfiles/alpha-sys-root/sbin/loader",argv,envp,regs,mem,zero_bss_segs);
		return ld_load_prog("/home/jloew/.nfs/Desktop/m-sim-c++/sysfiles/alpha-sys-root/sbin/loader",argv,envp,regs,mem,zero_bss_segs);
	}
...
//	md_addr_t data_break = mem->ld_data_base + mem->ld_data_size;

	//seek to the beginning of the first section header, the file header comes first, followed by the optional header (this is the aouthdr), the size of the aouthdr is given in Fhdr.f_opthdr
	fseek(fobj, sizeof(ecoff_filehdr) + MD_SWAPH(fhdr.f_opthdr), 0);

	debug("processing %d sections in `%s'...", MD_SWAPH(fhdr.f_nscns), filename);

	//loop through the section headers
	long floc = ftell(fobj);
...
//	fprintf(stderr, "sim: loading %s\n", argv[0].c_str());
	FILE *fobj = fopen(argv[0].c_str(), "r");
	if(!fobj)
	{
		fprintf(stderr,"cannot open executable `%s'", argv[0].c_str());
		return EFAULT;
	...
	//Read initial header for type determiniation: Try both elf and ecoff
	Elf64_External_Ehdr elf_fhdr;
	if(fread(&elf_fhdr, 16, 1, fobj) < 1)
	{
	...
	if(efhdr.f_magic == MD_SWAPH(ECOFF_ALPHAMAGIC))
	{
		mem->ld_target_big_endian = false;
		int retval = load_ecoff_littlealpha(fobj, regs, mem, argv, envp, zero_bss_segs);
		if(retval)
		{
			return retval;
		}
	}
	else if((efhdr.f_magic == MD_SWAPH(ECOFF_EB_MAGIC)) || (efhdr.f_magic == MD_SWAPH(ECOFF_EL_MAGIC)) || (efhdr.f_magic == MD_SWAPH(ECOFF_EB_OTHER)) || (efhdr.f_magic == MD_SWAPH(ECOFF_EL_OTHER)))
	{
		fprintf(stderr,"M-Sim cannot run PISA binary `%s'", argv[0].c_str());
		return EACCES;
	}
	else if((elf_fhdr.e_ident[EI_MAG0] == ELFMAG0) && (elf_fhdr.e_ident[EI_MAG1] == ELFMAG1) && (elf_fhdr.e_ident[EI_MAG2] == ELFMAG2) && (elf_fhdr.e_ident[EI_MAG3] == ELFMAG3))
	{
		int retval = load_elf(fobj, regs, mem, argv, envp, zero_bss_segs);
		if(retval)
		{
			return retval;
		}
...
//	mem->ld_stack_base = mem->ld_text_base - (-16) - 409600;
	...
	if(mem->ld_text_base < 16384)
	{
		mem->ld_stack_base = 0x12000000 - 0x2000;
	}
...
//	sp = mem->ld_stack_base - MD_MAX_ENVIRON * 8;
	...
	if(sp > mem->ld_text_base)
	{
		...
		fprintf(stderr,"environment overflow, increase MD_MAX_ENVIRON in machine.h (stack - %llx, text_base - %llx\n", sp, mem->ld_text_base);
		if(mem->ld_text_base >= 0x2000)
		{
			fprintf(stderr,"This may be valid behavior, if so, remove this check in loader.c\n");
		}
		return E2BIG;
	}
