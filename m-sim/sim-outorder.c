...
void writeback(unsigned int core_num)
{
	...
	while((rs = cores[core_num].eventq_next_event(sim_cycle)))
	{
		...
		if(rs->recover_inst)
		{
			...
			cores[core_num].rollbackTo(contexts[rs->context_id],sim_num_insn,rs,1,int_reg_cnts);
...
void register_rename(unsigned int core_num)
{
	...
	while(n_renamed < cores[core_num].decode_width)
	{
		...
		{
			if(cores[core_num].reg_file.find_free_physreg(my_regs.dest) < 0 || (int_reg_cnts[disp_context_id] >= int_reg_cap))
			{
