...
class file_table_t
{
	public:
		...
		md_gpr_t get_sim_fd(md_gpr_t real_fd);		//Returns the simulated File Descriptor that is used by simulated contexts
		...
		//select handler
		md_gpr_t selecter(md_gpr_t nfds, fd_set * readfd, fd_set * writefd, fd_set * exceptfd, timeval * timeout);
